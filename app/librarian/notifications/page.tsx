...
import { useNotifications, useSystemNotifications, useNotificationStats, useMarkAsRead, useMarkAllAsRead, useDeleteNotification } from '@/hooks/useNotifications';
...
export default function LibrarianNotificationsPage() {
  ...
  // API hooks - using librarian-specific hooks
  ...
  const { data: systemNotificationsData, isLoading: systemLoading } = useSystemNotifications({
    page: currentPage,
    limit: 10,
  });
  ...
  const getNotificationsData = () => {
    if (activeTab === 'system') {
      return systemNotificationsData;
    }
    return notificationsData;
  };

  const notificationsResponse = getNotificationsData();
  const notifications = notificationsResponse?.notifications || [];
  const isLoading = activeTab === 'system' ? systemLoading : notificationsLoading;
  const error = false; // Keep simple for now
  ...
  const tabs = [
    { id: 'all' as NotificationTab, label: 'All', count: stats?.total || 0 },
  ...
  return (
    <div>
      ...
      <div className="mb-6 sm:mb-8">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm sm:text-base text-gray-600">Manage library notifications and alerts</p>
          </div>
          <button
      ...
      <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
        ...
        {/* Pagination */}
        {notificationsResponse?.pagination && notificationsResponse.pagination.totalPages > 1 && (
          <div className="p-4 border-t border-gray-200 flex items-center justify-between">
            <span className="text-sm text-gray-600">
              Page {notificationsResponse.pagination.page} of {notificationsResponse.pagination.totalPages}
            </span>
            <div className="flex items-center gap-2">
              <button
              ...
              <button
                onClick={() => setCurrentPage(prev => Math.min(notificationsResponse.pagination.totalPages, prev + 1))}
                disabled={currentPage === notificationsResponse.pagination.totalPages}
                className="px-3 py-1 text-sm border border-gray-300 rounded hover:bg-gray-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Next
              </button>
...
// LibrarianNotificationRow Component
interface LibrarianNotificationRowProps {
  notification: Notification;
...
function LibrarianNotificationRow({ 
  notification, 
  onMarkAsRead, 
  onDelete, 
  getRelativeTime, 
      ...
      <div className={`flex items-center gap-4 p-3 rounded-lg hover:bg-gray-50 transition-colors ${
        ...
        <div className="w-10 h-10 bg-gray-300 rounded-full flex items-center justify-center flex-shrink-0">
          <span className="text-sm font-medium text-gray-600">
            {notification.user?.name ? notification.user.name.charAt(0).toUpperCase() : 'S'}
          </span>
        </div>
              ...
              <div className="space-y-6">
                ...
                {notification.user && (
                  <div>
                    ...
                    <p className="text-gray-700">{notification.user.email}</p>
                  </div>
                )}
              </div>
